...
if (isset($_GET['add'])) {
    ...
    if (in_array($tipe, $ekstensi) === true) {
        if ($ukuranFile < 1048576) { //1 mb
            ...
            create("INSERT INTO kategori2 VALUES ('$kodebarang','$namabarang','$jenisbarang','$noregister','$merktype','$tahunpembelian','$asalusul','$harga','$kondisi','$keterangan','$foto')");
            $upload = move_uploaded_file($sumber, $lokasi);
            if ($upload) {
                echo '<script>
                        alert("Data Berhasil Ditambah")
                        window.location = "' . base_url('kategori2') . '";
                        </script>';
            } else {
                echo '<script>
                        alert("Data Gagal Diupload")
                        window.location = "' . base_url('tambahbarang2') . '";
                        </script>';
            }
        } else {
...
elseif (isset($_GET['edit'])) {
    ...
    if ($_FILES['foto']['name'] == '') {
        $query = mysqli_query($koneksi, "UPDATE kategori2 SET kodebarang='$kodebarang',namabarang='$namabarang', jenisbarang='$jenisbarang', noregister='$noregister', merktype='$merktype',  tahunpembelian='$tahunpembelian',asalusul='$asalusul',harga='$harga',kondisi='$kondisi',keterangan='$keterangan'  ");
        if ($query) {
            echo '<script>
            ...
            echo '<script>
                ...
                window.location = "' . base_url('editbarang2') . '?id=' . $nipAsli . '";
                </script>';
        }
        ...
        if (in_array($tipe, $ekstensi) === true) {
            if ($ukuranFile < 1048576) { //1 mb
                ...
                mysqli_query($koneksi, "UPDATE kategori2 SET kodebarang='$kodebarang',namabarang='$namabarang',foto_pegawai='$foto', jenisbarang='$jenisbarang', noregister='$noregister', merktype='$merktype', tahunpembelian='$tahunpembelian',asalusul='$asalusul',harga='$harga',kondisi='$kondisi',keterangan='$keterangan' WHERE kodebarang='$nipAsli' ");
                $lokasi = "../_assets/img/" . $foto;
                $upload = move_uploaded_file($sumber, $lokasi);
                if ($upload) {
                    echo '<script>
